package com.example.inventariooiubo.Grupo;
...
public class Listadogrupo {
    ...
    public void todos(ActionEvent event) {
        try (Connection connection = MySQLConnection.connectDB()) {
            String sql = "SELECT * FROM reactivos WHERE (privado = 0 OR comunes = 1) OR grupo IN (SELECT nombre FROM Login)";
    ...
    public void grupo(ActionEvent event) {
        try (Connection connection = MySQLConnection.connectDB()) {
            String sql = "SELECT * FROM reactivos WHERE privado = 0 OR grupo IN (SELECT nombre FROM Login) ORDER BY grupo";
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
            ResultSet resultSet = preparedStatement.executeQuery();
    ...
    public void buscar(KeyEvent event) {
        ...
        try (Connection connection = MySQLConnection.connectDB()) {
            ...
            if (!nombreBuscar.isEmpty() && !casbuscar.isEmpty() && !grupoBuscar.isEmpty()) {
                sql = "SELECT * FROM reactivos WHERE ((privado = 0 OR comunes = 1) OR grupo IN (SELECT nombre FROM Login)) AND (cas LIKE ? OR nombre LIKE ?) AND (grupo LIKE ? OR grupo IN (SELECT nombre FROM Login))";
                ...
                sql = "SELECT * FROM reactivos WHERE ((privado = 0 OR comunes = 1) OR grupo IN (SELECT nombre FROM Login)) AND (cas LIKE ? OR nombre LIKE ?)";
                statement = connection.prepareStatement(sql);
                statement.setString(1, "%" + casbuscar + "%");
                statement.setString(2, "%" + nombreBuscar + "%");
            } else if (!nombreBuscar.isEmpty()) {
                sql = "SELECT * FROM reactivos WHERE ((privado = 0 OR comunes = 1) OR grupo IN (SELECT nombre FROM Login)) AND (nombre LIKE ?)";
                statement = connection.prepareStatement(sql);
                statement.setString(1, "%" + nombreBuscar + "%");
            } else if (!casbuscar.isEmpty()) {
                sql = "SELECT * FROM reactivos WHERE ((privado = 0 OR comunes = 1) OR grupo IN (SELECT nombre FROM Login)) AND (cas LIKE ?)";
                statement = connection.prepareStatement(sql);
                statement.setString(1, "%" + casbuscar + "%");
            } else if (!grupoBuscar.isEmpty()) {
                sql = "SELECT * FROM reactivos WHERE ((privado = 0 OR comunes = 1) OR grupo IN (SELECT nombre FROM Login)) AND (grupo LIKE ?)";
                statement = connection.prepareStatement(sql);
    ...
    public void EntrarNotas(ActionEvent event) {
        try {
            ...
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/example/inventariooiubo/Grupo/Notagrupo.fxml"));
    ...
    public void EntrarReactivo(ActionEvent event) {
        try {
            ...
            // Cargar el FXML de AdmGrupo.fxml
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/example/inventariooiubo/Grupo/Grupo.fxml"));
            Parent root = loader.load();
