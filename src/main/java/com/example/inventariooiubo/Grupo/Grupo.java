...
public class Grupo {
    ...
    private void crearReactivo(ActionEvent event) {
        ...
        try (Connection connection = MySQLConnection.connectDB()) {
            ...
            try {
                ...
                String checkGroupNameQuery = "SELECT * FROM Login WHERE nombre = ?";
    ...
    private void comprobarIdBorrar(ActionEvent event) {
        ...
        try (Connection connection = MySQLConnection.connectDB()) {
            String sql = "SELECT r.* FROM reactivos r " +
                    "JOIN Login l ON r.grupo = l.nombre " +
                    "WHERE r.id = ? ";
    ...
    private void Comprobar(ActionEvent event) {
        ...
        try (Connection connection = MySQLConnection.connectDB()) {
            String sql = "SELECT r.* FROM reactivos r " +
                    "LEFT JOIN Login l ON r.grupo = l.nombre " +
                    "WHERE r.id = ? AND (r.comunes = '1' OR r.grupo =l.nombre)";
            PreparedStatement statement = connection.prepareStatement(sql);
            statement.setString(1, idReactivo);
            ResultSet resultSet = statement.executeQuery();
    ...
    private void actualizarReactivo(ActionEvent event) {
        ...
        // Realizar la consulta SQL para verificar si el grupo coincide con un nombre en la tabla Login
        try (Connection connection = MySQLConnection.connectDB()) {
            String sql = null;
            if (grupoValue.isEmpty()) {

            } else {
                // Si el campo grupo no está vacío, busca coincidencias en la tabla Login
                sql = "SELECT * FROM Login WHERE nombre = ?";
                PreparedStatement statement = connection.prepareStatement(sql);
                statement.setString(1, grupoValue);
                ResultSet resultSet = statement.executeQuery();
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public void CerrarSesion(ActionEvent event) {
        try {
            ...
            System.out.println("Intentando borrar registros de la tabla Login...");

            // Ejecutar la operación DELETE
            try (Connection connection = MySQLConnection.connectDB();
                 PreparedStatement preparedStatement = connection.prepareStatement(sql)) {
                // Ejecutar la sentencia SQL para borrar todos los registros de la tabla
                int filasEliminadas = preparedStatement.executeUpdate();
                // Confirmar los cambios en la base de datos
                connection.commit();
                System.out.println("Se han eliminado " + filasEliminadas + " registros de la tabla Login.");
            } catch (SQLException e) {
                e.printStackTrace();
                // Manejar cualquier error de SQL que pueda ocurrir
            }

            // Después de ejecutar la operación DELETE, cambiar de escena
